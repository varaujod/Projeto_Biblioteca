...
class LivroRepository {
    ...
    async insereLivro(livro) {
        ...
        return new LivroEntity_1.LivroEntity(livro.titulo, livro.isbn, livro.autor, livro.editora, livro.edicao, livro.categoria);
    }
    validacaoISBN(isbn) {
    ...
    async filtraLivroPorISBN(isbn) {
        ...
        if (resultado && resultado.length > 0) {
            ...
            return new LivroEntity_1.LivroEntity(user.titulo, user.isbn, user.autor, user.editora, user.edicao, user.categoria);
        }
        return null;
    }
    ...
    async listarLivros() {
        ...
        if (resultado && resultado.length > 0) {
            for (let i = 0; i < resultado.length; i++) {
                ...
                livros.push(new LivroEntity_1.LivroEntity(user.titulo, user.isbn, user.autor, user.editora, user.edicao, user.categoria));
            }
        }
        return livros;
    }
